...
const loadOrders = async (req, res) => {
  try {
    ...
    // Filter by status - special handling for returned items
    if (status) {
      if (status === 'Returned') {
        // Filter orders that have at least one returned item
...
const updateOrderStatus = async (req, res) => {
  try {
    ...
    } else if (newIndex < currentIndex && status !== 'Pending') {
...
const updateItemStatus = async (req, res) => {
  try {
    ...
    } else if (itemStatuses.some(s => s === 'Out for Delivery')) {
...
const handleReturnRequest = async (req, res) => {
  try {
    ...
    if (action === 'approve') {
      ...
      // Add refund amount to user's wallet using wallet schema
      ...
      res.json({
        success: true,
        message: `Return approved. ₹${refundAmount.toLocaleString()} refunded to user's wallet.`,
